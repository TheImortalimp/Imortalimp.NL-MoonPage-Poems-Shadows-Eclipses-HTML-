...
Where the Moon is relative to the shadow axis, right now. Gamma gives the
...
σ(t) = √(σ_min² + (rate · Δt)²).
How big the shadow is. Straight from the true Earth–Moon and Earth–Sun
...
Which way the shadow lies. The offset is decomposed along the ecliptic
...
Using it
Status line under the full-Moon countdown: 🌍 Partial lunar eclipse · 96% of the disc is inside Earth's umbra, or, when nothing is happening, a ticking
...
nearest one with a real bite (umbral magnitude ≥ 0.25) at ~420×, opening at first
contact and looping back there. Click again to return to live time.
