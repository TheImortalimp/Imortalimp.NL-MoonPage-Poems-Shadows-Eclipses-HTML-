...
(async () => {
  ...
  await page.evaluate(() => {
    const R = Date; let base = new R('2026-08-28T01:20:00Z').valueOf(); const start = R.now();
    function F(...a) {
    ...
    window.__probe = () => document.getElementById('penumbraShadow').getAttribute('cx');
  });
  for (let i = 0; i < 8; i++) {
    await new Promise(r => setTimeout(r, 1200));
    const o = await page.evaluate(() => ({
      clock: new Date().toISOString().slice(11, 19),
      status: document.getElementById('eclipseStatus').textContent.slice(0, 58),
